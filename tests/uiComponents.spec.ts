...
test.describe("Form Layout page @block", () => {
    ...
    test("radio buttons", async ({ page }) => {
        ...
        expect(
            await usingTheGridForm.getByLabel("Option 2").isChecked()
        ).toBeTruthy();
    });
});
...
test("list and dropdowns", async ({ page }) => {
    ...
    page.getByRole("list"); // used for UL
    page.getByRole("listitem"); //used for LI elements
...
test("web tables", async ({ page }) => {
    ...
    const targetedRow = page.getByRole("row", { name: "twitter@outlook.com" });
    ...
    const targetedRowById = page
        .getByRole("row", { name: "11" })
        .filter({ has: page.locator("td").nth(1).getByText("11") });
    await targetedRowById.locator(".nb-edit").click();
    ...
    await expect(targetedRowById.locator("td").nth(5)).toHaveText(
        "test@test.com"
    );
...
test("datepicker", async ({ page }) => {
    ...
    let calendarMonthAndYear: any = await page
...
test("sliders", async ({ page }) => {
    ...
    await tempGauge.evaluate((node) => {
        node.setAttribute("cx", "231.90619362896732");
        node.setAttribute("cy", "231.9061936289673");
    });
    ...
    const box: {
        ...
        width: number;
        height: number;
    } | null = await tempBox.boundingBox();
    if (box === null) throw new Error("Size element error");
